...
const pick = await page.evaluate(() => {
  const nodes = [...document.querySelectorAll("div,span,li,a,button")];
  let el = nodes.find((e) => (e.textContent || "").trim() === "https://zovus.ru/");
  if (!el) el = nodes.find((e) => (e.textContent || "").trim() === "https://zovus.ru");
  if (el) {
    el.click();
...
await snap("pick-01");
writeFileSync(join(OUT, "pick-01.html"), await page.content());

if (/Подтверждение|HTML-файл|HTML file|ПОДТВЕРДИТЬ/i.test(t)) {
  await page.evaluate(() => {
    const n = [...document.querySelectorAll("div,span,button")].find((e) =>
      /^(HTML-файл|HTML file)$/i.test((e.textContent || "").trim())
    );
    if (n) n.click();
  });
  ...
  console.log("buttons", buttons.slice(0, 60));
  ...
  console.log("after", page.url(), t.slice(0, 2000));
  await snap("pick-02");
  await page.evaluate(() => {
    const b = [...document.querySelectorAll("button")].find((x) =>
