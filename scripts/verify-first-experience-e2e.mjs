...
(async()=>{const client=new pg.Client({connectionString:env.DATABASE_URL});await client.connect();await client.query("INSERT INTO user_accounts(id,email,name,token_version,age_confirmed_at,terms_accepted_at) VALUES('33333333-3333-4333-8333-333333333333','first-experience-e2e@example.invalid','E2E Test',0,now(),now()) ON CONFLICT(id) DO UPDATE SET token_version=0");await client.end();
const r=cp.spawnSync(process.execPath,['node_modules/@playwright/test/cli.js','test','--project=first-experience','--workers=1'],{env,stdio:'inherit'});const cleanup=new pg.Client({connectionString:env.DATABASE_URL});await cleanup.connect();await cleanup.query("DELETE FROM user_accounts WHERE id='33333333-3333-4333-8333-333333333333' AND email='first-experience-e2e@example.invalid'");await cleanup.end();process.exit(r.status??1);})().catch(e=>{console.error(e.message);process.exit(1)});
