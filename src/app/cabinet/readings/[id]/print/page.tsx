...
export default async function SavedReadingPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  await recordProductActivity(auth.profileUserId, "result_reopened", id, { product: report.kind });
  ...
  ]} visual={<ReadingSnapshot kind={report.kind} snapshot={report.snapshot} />} sections={[]} legacyContent={report.body} returnHref="/cabinet?tab=history"
