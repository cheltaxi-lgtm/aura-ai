...
export default function ReadingJourney({readingId,context}:{readingId?:string;context?:string}) {
  ...
  useEffect(()=>{
    ...
    const observer=new IntersectionObserver(entries=>{
      ...
      continuationTracked.current=key;
      void fetch("/api/diary/journey",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:"continuation_shown",readingId:journey.reading.id,continuationId:continuation.id})}).catch(()=>undefined);
      observer.disconnect();
    },{threshold:[0.5]});
  ...
  return <section className="ym-hide-content my-6 min-w-0 rounded-2xl border border-aura-gold/25 bg-gradient-to-br from-aura-gold/10 to-transparent p-4 sm:p-6" aria-labelledby={`${id}-title`}>
    ...
    <button type="button" className="btn-luxe btn-luxe--gold mt-3 min-h-11 w-full sm:w-auto" onClick={()=>void save()} disabled={busy}>{busy?"Сохраняем…":"Сохранить вывод"}</button>
    <p role="status" className="mt-2 text-sm text-white/75">{message}</p>
    <details className="mt-3"><summary className="min-h-11 cursor-pointer py-3 text-sm text-aura-gold">Добавить шаг, изменения или напоминание</summary><div className="mt-4 space-y-4">
