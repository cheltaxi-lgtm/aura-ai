...
export default function DestinyMatrixPreview() {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    startTransition(() => {
      const result = buildMatrixFreeSummary(birthDate, { name: name || undefined });
      if (!result) {
        setSummary(null);
  ...
  return (
    <div id="calculate" className="destiny-matrix-preview mt-10 scroll-mt-24">
      ...
      <p className="mt-2 text-sm text-white/55">
        Нужна только дата рождения. Базовый результат — сразу на экране, без регистрации. Полный
        AI-разбор с Эвелиной — от {PRICING.NUMEROLOGY_SESSION} ᚢ.
      </p>
      ...
      <form onSubmit={onSubmit} className="mt-5 space-y-3">
        ...
        >
          {pending ? "Считаем…" : "Рассчитать бесплатно"}
        </button>
      </form>
