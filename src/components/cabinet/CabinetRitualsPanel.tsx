  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (statsRes.ok) {
        ...
        const s = data.stats as CabinetRitualStats;
        setStats(s);
        onStatsLoaded?.(s);
      }
    } finally {
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {r.status === "reviewed" && r.outcomeRating ? (
                          <p className="mt-1 text-xs text-amber-300/80">
                            {"★".repeat(r.outcomeRating)}
                            {"☆".repeat(3 - r.outcomeRating)}
                          </p>
                        ) : null}
                        {awaiting ? (
        ...
        <div className="cabinet-rituals-empty">
          ...
          {showMasterPicker ? (
            ...
            <button
              ...
              className="cabinet-btn cabinet-btn--primary mt-5"
            >
              Заказать первый обряд
