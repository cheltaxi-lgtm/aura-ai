...
export const JOURNEY_SOURCES_SQL = `
  SELECT id,created_at,CASE context_data->>'type' WHEN 'aura_reading' THEN 'aura' WHEN 'palm_reading' THEN 'palm' ELSE 'tarot' END AS kind
...
const ACTIONS: Record<string,RuneActionType> = { matrix:"NUMEROLOGY_SESSION", natal:"NATAL_READING", human_design:"HD_REPORT", matrix_compatibility:"MATRIX_PAIR_REPORT", aura:"AURA_READING", palm:"PALM_READING" };
...
export async function getReadingJourney(userId: string, requestedId?: string) {
  ...
  const pendingProducts:Record<string,string>={natal_interpretation:"natal",hd_report:"human_design",hd_composite_report:"human_design",numerology_reading:"matrix",aura_reading:"aura",palm_reading:"palm"};
  for(const job of pending.rows)if(pendingProducts[job.kind])owned.add(pendingProducts[job.kind]);
  const context = (["tarot","ritual","numerology"].includes(source.kind) ? "aura" : source.kind) as CrossProductContext;
  ...
  const continuation = rec ? {...rec,benefit:BENEFITS[rec.product],cost:settings.costs[ACTIONS[rec.product]],rubPerRune:settings.rubPerRune} : null;
