/** Same IANA zone as AURA_DAY_TIMEZONE — kept here so the client never imports the DB service. */
export const AURA_CADENCE_TZ = "Europe/Moscow";

function moscowDayKey(at: Date): string {
  return new Intl.DateTimeFormat("en-CA", {
    timeZone: AURA_CADENCE_TZ,
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
  }).format(at);
...
export function nextAuraShotAt(now = new Date()): Date {
...
export function formatAuraWaitRu(now = new Date()): string {
  const ms = Math.max(0, nextAuraShotAt(now).getTime() - now.getTime());
