...
const PALM_READING: PaidJobKindConfig = {
  kind: "palm_reading",
  runeAction: "PALM_READING",
  maxActivePerUser: 1,
  timeoutMs: 240_000,
  waitPolicy: "background_notified",
  etaRangeSec: { min: 60, max: 180 },
  productTitle: "Разбор ладони по фото",
  workerPath: (job) => ({
    path: "/api/palm/report",
    body: { ...job.input, async: false },
  }),
  matchesWorkerPath: (pathname) => pathname === "/api/palm/report",
  buildDedupeKey: (userId, payload) =>
    hashDedupeParts([userId, "palm_reading", payload.palmSnapshotId ?? payload.idempotencyKey]),
};
