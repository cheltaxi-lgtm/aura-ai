...
export type AdminSessionMemoryRow = {
  id: string;
...
export async function listSessionMemoriesForUser(
  userId: string,
  limit = 100
): Promise<AdminSessionMemoryRow[]> {
  const { rows } = await query<{
    ...
    `SELECT id, session_id, character_key,
            TO_CHAR(session_date, 'DD.MM.YYYY') AS date,
            topic_summary, key_cards, prediction, mood, outcome_rating
     FROM session_memories
...
export async function deleteSessionMemory(userId: string, memoryId: string): Promise<boolean> {
  const res = await query(`DELETE FROM session_memories WHERE user_id = $1 AND id = $2`, [
    userId,
    memoryId,
  ]);
