...
const TOPIC_PATTERNS: Record<NumerologyTopic, RegExp> = {
  ...
  personal_cycle:
    /личн(ый|ого|ом|ые|ым|ую|ая)?\s*(год|месяц|день|недел)|этот\s+год|\bсегодня\b|(?:^|[\s,.!?])месяц(?:[\s,.!?]|$)|личн(ый|ого|ом)?\s*месяц|в\s+этом\s+месяце|на\s+недел|расклад\s+по\s+цифрам|энерг(ия|ии)\s+дня|что\s+меня\s+жд[ёе]т|жд[ёе]т\s+меня\s+сегодня|цикл\s+года/i,
  karma:
