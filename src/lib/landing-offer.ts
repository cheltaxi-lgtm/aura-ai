...
export function buildLandingOfferCopy(
  ...
  const heroTitle = "Что он думает и что чувствует? Три карты покажут без оплаты";
  const heroSubtitle =
  ...
  const pricingLine = config.enabled
    ? `3 карты бесплатно · ${freeQLabel} мастеру бесплатно · полная расшифровка — ${fmtPrice(config.costs.READING)}`
