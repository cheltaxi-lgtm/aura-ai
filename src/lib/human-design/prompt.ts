...
5) Раздел «Центры»: для КАЖДОГО определённого и каждого открытого центра — подзаголовок ### с названием центра и 1–2 абзаца. Раздел «Каналы»: для КАЖДОГО определённого канала — ### с ключом/названием канала. Раздел «Планеты и узлы»: ### для ключевых тел (Солнце, Земля, Луна, узлы, и ещё 2–3 важные). Раздел «Переменные и среда»: color/tone/base Солнца и подсказки познания/среды; не выдавай за медицинский PHS. Раздел «Сон и восстановление»: свяжи с центрами и средой; упомяни висящие ворота, если есть.
6) Не пропускай ни один ## из списка выше. Объём раздела — обычно 3–6 абзацев (у Центров/Каналов сумма ### длиннее). Списки — только если усиливают ясность, до 6 пунктов.
7) Запрещено: эмодзи; декоративные символы; таблицы; расписания по часам; HTML; ссылки; заголовки # (только ## и ###); вода и общие фразы вроде «все уникальны».
