...
const DEFAULT_GUEST_SPREAD_MASTER = "veronika";
...
export function resolveGuestSpreadMasterId(explicit?: string | null): string {
  ...
  return guest?.masterId?.trim() || DEFAULT_GUEST_SPREAD_MASTER;
}
...
export function buildRegisterHref(
  ...
  return buildAuthHref("/auth/user/register", withAppShellIfNeeded(destination), fallback);
}
