...
export function authProductCopy(returnTo: string, guestTarotResume = false): AuthProductCopy {
  ...
  const copy: Record<AuthProduct, Omit<AuthProductCopy, "product">> = {
    ...
    aura: { title: "Сохраните результат чтения ауры", subtitle: "После входа откроется тот же результат; файл фото на сервере не хранится. Полный разбор подтверждается отдельно." },
    palm: { title: "Сохраните результат чтения ладони", subtitle: "После входа откроется тот же результат; файл фото на сервере не хранится. Полный разбор подтверждается отдельно." },
