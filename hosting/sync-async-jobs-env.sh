...
umask 077
{
  ...
  for key in ASYNC_JOB_POLL_MS ASYNC_JOB_CONCURRENCY ASYNC_JOB_REQUEST_TIMEOUT_MS ASYNC_JOB_STALE_RUNNING_MS ASYNC_JOB_TIMEOUT_GRACE_MS ASYNC_JOB_KINDS; do
    val="$(value_of "$key")"
    if [ -n "$val" ]; then
      printf '%s=%s\n' "$key" "$val"
    fi
