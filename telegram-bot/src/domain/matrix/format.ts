...
const MATRIX_POINTS: Array<{ name: string; emoji: string; aliases?: string[] }> = [
  ...
  {
    name: "Точка возраста сейчас",
    emoji: "🪴",
    aliases: ["Точка возраста", "Возраст сейчас"],
  },
  {
...
export function formatMatrixPremiumTeaser(input: MatrixTeaserInput): string {
  ...
  const strip = (raw: string | null | undefined) =>
    (raw || "")
      .replace(/\s+/g, " ")
      .replace(
        /^(Денежный канал|Отношения|Аркан года|Зона комфорта|Кармический хвост|Точка возраста|урок)\s*:\s*/i,
        ""
      )
      .trim();
