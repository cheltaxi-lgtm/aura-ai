# Block A deploy — без migrations (SKIP_MIGRATIONS=1)
...
Write-Host ">>> Deploy on VM (SKIP_MIGRATIONS=1)..."
$DeployCmd = @'
set -e
...
cd /opt/aura-ai && SKIP_EXTRACT=1 SKIP_MIGRATIONS=1 bash /opt/aura-ai/proxmox-setup/vm_local_deploy.sh
